...
const PRODUCTS = {
  ...
  dawnlist: {
    ...
    ackEnglish:
      ...
      "nothing else. Subscriptions are not open yet. Dawnlist is on the Microsoft Store for Windows and installs free, but it needs a subscription to run. A Mac App Store edition has been submitted to Apple. There is no date for either.\n\n" +
...
const DAWNLIST_FACTS = `- Dawnlist is a desktop application for Windows for people looking for work. Dawnlist is software for individual job seekers, not a job board: employers cannot post vacancies, pay for placement or advertise through it. A Mac App Store edition has been submitted to Apple. It is not on sale, and there is no date for when it will be. Never say the Mac edition is approved or available, and never promise or estimate when it might be. It reads job feeds every morning, judges every posting against a fit brief it builds by interviewing the user, and hands back a ranked shortlist with the rejected postings still visible and the reason for each rejection stated.
- STATUS: SUBSCRIPTIONS ARE NOT OPEN YET. Dawnlist is ON THE MICROSOFT STORE for Windows and installs FREE, but it needs a subscription to run, so until subscriptions open it installs and then asks for a key nobody can obtain yet. Say both halves together, never just the first. A Mac App Store edition has been submitted to Apple but is NOT on sale and has NO date -- never say it is approved or available. There is NO DATE for subscriptions opening -- never estimate or imply one. The waiting list at https://dawnlist.spencerfields.com/ is the way to be told when subscriptions open.
- PRICING: $79 per month, a RECURRING SUBSCRIPTION, one plan. It is NOT a one-time purchase -- never say bought once, yours forever, or no subscription. The job feed is included in that price and needs no separate account with a job-data provider. SEPARATELY, the buyer supplies their own Anthropic API key and Anthropic bills them directly -- usually a pound or two a month, depending on how much they search. Say so plainly if asked. NEVER quote a more precise figure, never split it into reading and drafting, and never state a one-off setup cost: no such figures are measured. The reading and drafting costs are the BUYER'S, not Spencer's. The job feed is the opposite -- Spencer's cost, included in the $79. A second plan covers searches spanning many regions; its price is NOT decided, so never quote or guess one. Nothing is purchasable yet.
...
- IT REQUIRES THE USER'S OWN MODEL-PROVIDER ACCOUNT. Assessment and drafting run on an API key the user holds and enters during setup, kept in the operating system's credential store, and that provider bills the user directly for the usage, separately from whatever the application itself costs. This narrows who Dawnlist is for and is stated deliberately rather than discovered later. If asked whether a key is needed, say yes plainly. Do not estimate what the usage would cost.
- Data: postings, decisions, drafts and the tracker live in a database on the user's own machine and export on request. CAREER DATA NEVER PASSES THROUGH SPENCER FIELDS' INFRASTRUCTURE. The fit brief, factsheet and posting text do leave the machine for assessment, but they go straight to the model provider on the user's own account; there is no intermediary holding them. A route through one was built during development and deliberately removed. The one thing that does pass through a service of Spencer Fields' own is the job search itself -- a query and the postings that come back, which carry nobody's history. The full privacy policy is published before anything can be bought.
