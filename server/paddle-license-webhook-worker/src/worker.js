...
const PRODUCTS = {
  ...
  dawnlist: {
    ...
    ackEnglish:
      ...
      "nothing else. Dawnlist is on the Microsoft Store for Windows and installs free, but it needs a licence to run and licences are not on sale yet. There is no date for that. A Mac App Store edition has been submitted to Apple. It is not on sale, and there is no date for when it will be.\n\n" +
...
const DAWNLIST_FACTS = `- Dawnlist is a desktop application for Windows for people looking for work. A Mac App Store edition has been submitted to Apple. It is not on sale, and there is no date for when it will be. Never say the Mac edition is approved or available, and never promise or estimate when it might be. It reads job feeds every morning, judges every posting against a fit brief it builds by interviewing the user, and hands back a ranked shortlist with the rejected postings still visible and the reason for each rejection stated.
- STATUS: Dawnlist is ON THE MICROSOFT STORE for Windows and installs FREE, but it is NOT YET ON SALE: a licence is required to run it and licences cannot be bought yet, so it installs and then asks for a key nobody can obtain. Say both halves together, never just the first. A Mac App Store edition has been submitted to Apple but is NOT on sale and has NO date -- never say it is approved or available. There is NO DATE for purchasing opening -- never estimate or imply one. The waiting list at https://dawnlist.spencerfields.com/ is the way to be told when a licence can be bought.
